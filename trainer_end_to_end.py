...
def _to_rgb_image(t):
    """
    Convert a [3, H, W] tensor in [0, 1] to uint8 RGB [H, W, 3]
    """
...
class Trainer:
    def __init__(self, options):
        ...
        self.parameters_to_train_0 += list(
            self.models["position_encoder"].parameters()
        )
        ...
        if self.use_pose_net:
            ...
            if self.opt.learn_intrinsics:
                self.models["intrinsics_head"] = decoders.IntrinsicsHead(
                    self.models["pose_encoder"].num_ch_enc
                )
                self.models["intrinsics_head"].to(self.device)
                self.parameters_to_train += list(
                    self.models["intrinsics_head"].parameters()
                )

        if self.opt.predictive_mask:
            assert self.opt.disable_automasking, (
                "When using predictive_mask, please disable automasking "
                "with --disable_automasking"
            )
        ...
        self.get_occu_mask_backward = get_occu_mask_backward(
            (self.opt.height, self.opt.width)
        ).to(self.device)

        self.get_occu_mask_bidirection = get_occu_mask_bidirection(
            (self.opt.height, self.opt.width)
        ).to(self.device)
        ...
        for scale in self.opt.scales:
            ...
            self.position_depth[scale] = optical_flow(
                (h, w), self.opt.batch_size, h, w
            ).to(self.device)
        ...
        print("Using split:\n  ", self.opt.split)
        print(
            "There are {:d} training items, {:d} validation items and {:d} testing items\n".format(
    ...
    def set_train_0(self):
        """Convert all models to training mode for position net only."""
    ...
    def set_train(self):
        """Convert all models to training mode (main stage)."""
        for param in self.models["position_encoder"].parameters():
            param.requires_grad = False
        for param in self.models["position"].parameters():
            param.requires_grad = False
    ...
    def process_batch_0(self, inputs):
        """Pass a minibatch through the position network and compute losses."""
    ...
    def predict_poses_0(self, inputs):
        """Predict positions for registration stage."""
        outputs = {}
        if self.num_pose_frames == 2:
            ...
            for f_i in self.opt.frame_ids[1:]:
                if f_i != "s":
                    ...
                    # position
                    position_inputs = self.models["position_encoder"](
                        torch.cat(inputs_all, 1)
                    )
                    position_inputs_reverse = self.models["position_encoder"](
                        torch.cat(inputs_all_reverse, 1)
                    )
                    outputs_0 = self.models["position"](position_inputs)
                    outputs_1 = self.models["position"](position_inputs_reverse)

                    for scale in self.opt.scales:
                        outputs[("position", scale, f_i)] = outputs_0[
                            ("position", scale)
                        ]
                        ...
                        outputs[("position_reverse", scale, f_i)] = outputs_1[
                            ("position", scale)
                        ]
                        ...
                        outputs[("occu_mask_backward", scale, f_i)], _ = (
                            self.get_occu_mask_backward(
                                outputs[("position_reverse", "high", scale, f_i)]
                            )
                        )
                        outputs[("occu_map_bidirection", scale, f_i)] = (
                    ...
                    # transform
                    transform_input = [
                        outputs[("registration", 0, f_i)],
                        inputs[("color", 0, 0)],
                    ]
                    transform_inputs = self.models["transform_encoder"](
                        torch.cat(transform_input, 1)
                    )
                    outputs_2 = self.models["transform"](transform_inputs)

                    for scale in self.opt.scales:
                        outputs[("transform", scale, f_i)] = outputs_2[
                            ("transform", scale)
                        ]
    ...
    def compute_losses_0(self, inputs, outputs):
        """Compute losses for the position stage."""
        losses = {}
        total_loss = 0
        ...
        for scale in self.opt.scales:
            ...
            loss += (
                self.opt.position_smoothness
                * (loss_smooth_registration / 2.0)
                / (2 ** scale)
            )
    ...
    def process_batch(self, inputs):
        """Pass a minibatch through the main network and generate images and losses."""
        for key, ipt in inputs.items():
            inputs[key] = ipt.to(self.device)
            ...
            for f_i in self.opt.frame_ids[1:]:
                if f_i != "s":
                    ...
                    for scale in self.opt.scales:
                        ...
                        outputs[("occu_mask_backward", scale, f_i)], outputs[
                            ("occu_map_backward", scale, f_i)
                        ] = self.get_occu_mask_backward(
                            outputs[("position_reverse", "high", scale, f_i)]
                        )
                        outputs[("occu_map_bidirection", scale, f_i)] = (
                    ...
                    # pose
                    pose_inputs = [
                        self.models["pose_encoder"](torch.cat(inputs_all, 1))
                    ]
                    axisangle, translation, intermediate_feature = self.models[
                        "pose"
                    ](pose_inputs)
    ...
    def generate_images_pred(self, inputs, outputs):
        ...
        for scale in self.opt.scales:
            ...
            for i, frame_id in enumerate(self.opt.frame_ids[1:]):
                ...
                outputs[("position_depth", scale, frame_id)] = (
                    self.position_depth[source_scale](cam_points, cam_K, T)
                )
    ...
    def process_batch_val(self, inputs):
        """Pass a minibatch through the network for validation."""
        for key, ipt in inputs.items():
            inputs[key] = ipt.to(self.device)
        outputs = self.models["depth_model"](inputs["color_aug", 0, 0])
    ...
    def compute_losses_val(self, inputs, outputs):
        """Compute NCC-based validation loss."""
        losses = {}
        total_loss = 0
        ...
        for scale in self.opt.scales:
            ...
            for frame_id in self.opt.frame_ids[1:]:
                registration_losses.append(
                    ncc_loss(
                        outputs[("registration", scale, frame_id)].mean(1, True),
                        target.mean(1, True),
                    )
                )

            registration_losses = torch.cat(registration_losses, 1)
            registration_losses, idxs_registration = torch.min(
                registration_losses, dim=1
            )
    ...
    def log_time(self, batch_idx, duration, loss):
        ...
        training_time_left = (
            self.num_total_steps / self.step - 1.0
    ...
    def log(self, mode, inputs, outputs, losses):
        """Write an event to TensorBoard + W&B."""
        # ---------------- TensorBoard scalars ----------------
        ...
        # ---------------- TensorBoard images -----------------
        for j in range(min(4, self.opt.batch_size)):  # write a maximum of 4 images
